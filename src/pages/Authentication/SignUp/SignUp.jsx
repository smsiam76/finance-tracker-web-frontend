...
const SignUp = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // send data to database through useUsers hook
      const res = await createDBUser(userInfo);

      if (res?.insertedId) {
        toast.success(`${fullName} Account created successfully!`);
        navigate("/dashboard");
      }
    } catch (error) {
      setLoading(false);
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary cursor-pointer border text-white py-3 rounded-xl font-semibold text-sm hover:bg-white hover:border-primary hover:text-primary transition shadow-md shadow-emerald-100 mt-2"
            >
              {isSubmitting ? (
